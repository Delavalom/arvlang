...
func testEval(input string) value.Object {
	l := lexer.New(input)
	p := parser.New(l)
...
func testIntegerObject(t *testing.T, obj value.Object, expected int64) bool {
...
func testBooleanObject(t *testing.T, obj value.Object, expected bool) bool {
	result, ok := obj.(*value.Boolean)
	if !ok {
...
func testNullObject(t *testing.T, obj value.Object) bool {
	if obj != NIL {
...
func testErrorObject(t *testing.T, obj value.Object, expected any) bool {
	result, ok := obj.(*value.Error)
	errResult := expected.(string)
	if !ok {
		t.Errorf("object is not Error. got=%T (%+v)", obj, obj)
		return false
	}
	if result.Message != errResult {
		t.Errorf("error object has wrong message. got=%s, want=%s",
			result.Message, errResult)
		return false
	}
	return true
}
	...
	input := `let two = "two";
{
	...
	evaluated := testEval(input)
	result, ok := evaluated.(*value.Hash)
	if !ok {
